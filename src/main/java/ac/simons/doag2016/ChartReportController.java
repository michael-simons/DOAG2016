...
public class ChartReportController {
    ...
    public void getCharts(
        ...
        final CommonTableExpression<Record3<BigInteger, Integer, Integer>> currentMonth = name("current_month").fields("track_id", "cnt", "position")
        ...
        final CommonTableExpression<Record3<BigInteger, Integer, Integer>> previousMonth = name("previous_month").fields("track_id", "cnt", "position")
                .as(select(PLAYS.TRACK_ID, count().as("cnt"), denseRank().over(orderBy(count().desc())).as("position"))
                        .from(PLAYS)
                        .where(playedOnTruncatedToDay.between(Date.valueOf(hlp.atDay(1)), Date.valueOf(hlp.atEndOfMonth())))
                        .groupBy(PLAYS.TRACK_ID));
